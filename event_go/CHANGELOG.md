...
- 修复 5 倍峰值压测下 SQLite 读路径 P95=520.1ms 超标的尾延迟问题：将固定单连接（`SetMaxOpenConns(1)`）提升为多连接（`dbMaxOpenConns=16`），并通过 DSN `_pragma` 注入 `busy_timeout` 与 `foreign_keys`，写入正确性由应用层互斥锁串行化保证；复测 P95=1.0ms、P99=48.4ms、错误请求 0。
...
- 未来 schema 版本 fail-closed：迁移拒绝 `version > CurrentSchemaVersion` 的数据库（拒绝迁移并提示库由更新版本应用生成），readiness 探针上报 `schema=incompatible` 并返回 503，避免以不兼容的数据库进入服务。
