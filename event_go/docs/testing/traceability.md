...
| BUG-G4-004 | v6.0 | P1 | JWT 分隔符洪泛不得触发 `GO-2025-3553` 的过量内存分配；可达 Go 漏洞必须阻断 CI | `jwt/v5.2.2`、pinned `govulncheck@v1.6.0` CI gate | REG-BUG-G4-001、SEC-GOVULN-001 | — | v6.0 test-report / Commit 与 Run 待回填 | Local Candidate |
| BUG-G4-005 | v6.0 | P1 | 构建、测试和发布不得使用存在可达标准库漏洞的 Go patch；`go-version-file` 必须固定安全 patch | `go 1.25.12`、setup-go、pinned govulncheck | SEC-GO-TOOLCHAIN-001、SEC-GOVULN-001 | — | v6.0 test-report / 首次 Run 29433701071 正确阻断，修复 Run 待回填 | Local Candidate |
