...
| G5-R02 foundation | v6.1 | Critical | active owner/admin 才能创建非 owner 邀请；每组织最多一个 active owner；接受者邮箱必须匹配登录邮箱或已验证恢复邮箱；过期与重复消费拒绝 | Schema v12、OrganizationMember/Invitation Store、ADR-004 | DOM-ORG-MEMBER-001、SEC-ORG-INVITE-001 | 自助邀请 API/UI 待 G5.4 | v6.1 test-report / Commit f548a29 / Run 29437366319 | Partial Remote Pass |
...
| G5-R04 foundation | v6.1 | Critical | platform_admin 与 organization_member principal/凭证互不替代；管理前端验证 principal_type；租户入口可独立关闭 | PlatformAdminAuth、organization context、admin.ts、ORGANIZATION_AUTH_ENABLED、ADR-005 | SEC-PRINCIPAL-SEPARATION-001、SEC-TENANT-FLAG-001、CT-API-TENANT-AUTH-001、FE-PLATFORM-PRINCIPAL-001 | platform 业务路由已复用 scoped Service，完整替换待 G5.4 | v6.1 test-report / Commit 780c496 / Run 29440070376 | Partial Remote Pass |
...
| G5-R05 resources | v6.1 | Critical | Event/Ticket/Registration/Admission/Checkin/Export 的管理查询和写入在 SQL 与统一 Service 中强制 tenant scope；正确资源 ID 不能绕过 organization path；五角色业务路由矩阵生效 | OrganizationOperationsService、scoped Store、tenant routes、OpenAPI、ADR-006 | SEC-TENANT-RESOURCE-SCOPE-001、SEC-TENANT-BUSINESS-MATRIX-001、SEC-TENANT-EXPORT-001、CT-API-TENANT-RESOURCES-001 | G5.3 后端 API；自助运营 UI 待 G5.4 | v6.1 test-report / Commit 00ebef0 / Run 29443464930 | Remote Pass |
